...
ni = int(np.sqrt(config.train.batch_size)+0.99)
...
def prepare_nn_data(hr_img_list, lr_img_list, mode = "RGB", chanels = {0,1, 2}, idx_img=None):
    i = np.random.randint(len(hr_img_list)) if (idx_img is None) else idx_img

    input_image  = get_imgs_fn(lr_img_list[i], mode, chanels)
    output_image = get_imgs_fn(hr_img_list[i], mode, chanels)

    scale        = int(output_image.shape[0] / input_image.shape[0])
    assert scale == config.model.scale

    out_patch_size = patch_size * scale
    input_batch  = np.empty([batch_size,patch_size,patch_size, len(chanels)])
    output_batch = np.empty([batch_size,out_patch_size,out_patch_size,len(chanels)])
...
def train(device):
    ...
    if(device >= 0 ):
        device_name = '/device:GPU:{}'.format(device)
    else:
        device_name = '/device:CPU:0'

    ###========================== DEFINE MODEL ============================###
    #with tf.device(device_name):
    t_image = tf.placeholder('float32', [batch_size, patch_size, patch_size, config.model.input_channels], name='t_image_input')
    t_target_image = tf.placeholder('float32', [batch_size, patch_size*config.model.scale, patch_size*config.model.scale, config.model.input_channels], name='t_target_image')
    t_target_image_down = tf.image.resize_images(t_target_image, size=[patch_size*2, patch_size*2], method= 0 , align_corners=False) 

    net_image, net_feature, net_gradient = LapSRN2x(t_image, is_train=True, reuse=False)

    ## test inference
    net_image_test, net_grad_test, _ = LapSRN2x(t_image, is_train=False, reuse=True)

    ###========================== DEFINE TRAIN OPS ==========================###
    loss2   = compute_charbonnier_loss(net_image.outputs, t_target_image, is_mean=True)
    g_loss  = loss2
    ...
    tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir+'/params_{}.npz'.format(tl.global_flag['mode']), network=net_image)
    ...
    ###========================== INTERMEDIATE RESULT ===============================###
    sample_ind = 37
    sample_input_imgs,sample_output_imgs = prepare_nn_data(valid_hr_list,valid_lr_list, "YCbCr", {0}, sample_ind)
    ...
    for epoch in range(config.train.n_epoch):
        ...
        for idx_file in range(len(idx_list)):
            step_time = time.time()
            batch_input_imgs,batch_output_imgs = prepare_nn_data(train_hr_list,train_lr_list, "YCbCr", {0}, idx_file)
        ...
        if (epoch >= 0):
            tl.files.save_npz(net_image.all_params,  name=checkpoint_dir+'/params_{}.npz'.format(tl.global_flag['mode']), sess=sess)
...
def train4x():
    ...
    net_image2, net_grad2, net_image1, net_grad1 = LapSRN4x(t_image, is_train=True, reuse=False)
    net_image2.print_params(False)

    ## test inference
    net_image_test, net_grad_test, _, _ = LapSRN4x(t_image, is_train=False, reuse=True)
    ...
    ###========================== INTERMEDIATE RESULT ===============================###
    sample_ind = 37
    sample_input_imgs,sample_output_imgs = prepare_nn_data(valid_hr_list,valid_lr_list,idx_img = sample_ind)
    tl.vis.save_images(truncate_imgs_fn(sample_input_imgs),  [ni, ni], save_dir+'/train_sample_input.png')
    tl.vis.save_images(truncate_imgs_fn(sample_output_imgs), [ni, ni], save_dir+'/train_sample_output.png')
    ...
    for epoch in range(config.train.n_epoch):
        ...
        for idx_file in range(len(idx_list)):
            step_time = time.time()
            batch_input_imgs,batch_output_imgs = prepare_nn_data(train_hr_list,train_lr_list, idx_img= idx_file)
            errM, _ = sess.run([g_loss, g_optim], {t_image: batch_input_imgs, t_target_image: batch_output_imgs})
            total_g_loss += errM
            n_iter += 1
...
def test(file):
    try:
        ...
        scaled_ycbcr_image = convert_rgb_to_ycbcr(
                resize_image_by_pil(input_image, config.model.scale))

        input_y_img = input_image[:,:,0]

        size = input_y_img.shape
        print('Input size: %s,%s,%s'%(size[0],size[1],size[2]))
        t_image = tf.placeholder('float32', [None,size[0],size[1],1], name='input_image')
        #net_g, _, _, _ = LapSRN(t_image, is_train=False, reuse=False)
        net_g, _, _  = LapSRN2x(t_image, is_train=False, reuse=False)
        ...
        out = sess.run(net_g.outputs, {t_image: [input_y_img]})
        print("took: %4.4fs" % (time.time() - start_time))

        out_y_img = truncate_imgs_fn(out[0,:,:,:])

        out_img = convert_y_and_cbcr_to_rgb(out_y_img, scaled_ycbcr_image[:,:,1:3])
    
        tl.files.exists_or_mkdir(save_dir)
        tl.vis.save_image(out_img, save_dir+'/test_out.png')
        tl.vis.save_image(input_image, save_dir+'/test_input.png')

def validate(valid_lr_list, valid_hr_list):
    file = valid_lr_list[0]
    try:
        ...
        ###======================= TEST =============================###
        for f, f1 in valid_lr_list, valid_hr_list:
            img = get_imgs_fn(f)
            input_image = normalize_imgs_fn(img)
            hr_img = get_imgs_fn(f1)
            gt_img = normalize_imgs_fn(hr_img)

            errM, _ = sess.run([g_loss, g_optim], {t_image: batch_input_imgs, t_target_image: batch_output_imgs})
            out = sess.run(net_g.outputs, {t_image: [input_image], t_target_image: gt_img} )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-g','--gpu', choices=[-1, 0, 1], default=-1, help='select device, CPU:-1, GPU: GPU_ID')
